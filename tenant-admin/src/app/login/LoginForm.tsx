...
export default function LoginForm({
  ...
  const bgImage = loginBackgroundUrl || "/bg.jpg";
  const title = displayName ? `Welcome to ${displayName}` : "Welcome Back";

  return (
    <div
      className="min-h-screen flex items-center justify-center px-4 py-12 relative overflow-hidden"
      style={{
        background: `linear-gradient(to bottom, rgba(0,0,0,0.5) 0%, color-mix(in srgb, var(--teal-600) 60%, transparent) 50%, rgba(0,0,0,0.85) 100%), url('${bgImage}') center/cover no-repeat`,
        backgroundColor: "#0a0a0a",
      }}
    >
      <div className="w-full max-w-md relative z-10">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-24 h-24 rounded-2xl bg-white/10 backdrop-blur border border-white/20 shadow-lg mb-4 overflow-hidden">
            {logoSrc.startsWith("http") ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={logoSrc} alt="" width={80} height={80} className="object-contain p-2 w-20 h-20" />
            ) : (
              <Image
                src={logoSrc}
                alt="Logo"
