...
type CreateDriverResult = { success?: boolean; error?: string } | void;

type Props = {
  createDriver: (formData: FormData) => Promise<CreateDriverResult>;
};

export function CreateDriverModal({ createDriver }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      if (result?.success) {
        ...
        setOpen(false);
        form.reset();
        router.push("/drivers?success=" + encodeURIComponent("Driver created"));
        router.refresh();
      }
    });
  }
          ...
          <div className="relative w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl dark:border-zinc-800 dark:bg-zinc-900">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <h2 id="create-driver-title" className="text-lg font-semibold text-zinc-900 dark:text-zinc-50">
                  Create New Driver
                </h2>
              </div>
              <button
