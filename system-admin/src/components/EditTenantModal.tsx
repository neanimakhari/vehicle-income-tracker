...
export function EditTenantModal({ tenant, onClose, updateTenant }: EditTenantModalProps) {
  ...
  const [tab, setTab] = useState<"details" | "brand">("details");
  ...
  return (
    ...
    >
      ...
      >
        ...
        {tab === "brand" ? (
          <BrandStudioPanel tenantId={tenant.id} />
        ) : (
          <form action={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              <CopyLinkRow
                label="Tenant admin login"
                url={tenantAdminLoginUrl(tenant.slug)}
              />
              <CopyLinkRow
                label="Driver app download"
                url={vitAppDownloadUrl(tenant.slug)}
              />
            </div>
            <div>
