...
type CreatePlatformAdminModalProps = {
  isOpen: boolean;
  onClose: () => void;
  createPlatformAdmin: (formData: FormData) => Promise<{ success: boolean; error?: string }>;
};

export function CreatePlatformAdminModal({
  isOpen,
  onClose,
  createPlatformAdmin,
}: CreatePlatformAdminModalProps) {
  const router = useRouter();
  const [error, setError] = useState<string | null>(null);
  const [showPassword, setShowPassword] = useState(false);
  const dialogRef = useRef<HTMLDivElement>(null);
  const previousActiveRef = useRef<HTMLElement | null>(null);
  const titleId = "create-platform-admin-modal-title";
  ...
  async function handleSubmit(formData: FormData) {
    ...
    if (result?.success) {
      handleClose();
      router.push("/platform-admins?success=" + encodeURIComponent("Platform admin created"));
      router.refresh();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby={titleId} ref={dialogRef}>
      ...
      <div className="relative w-full max-w-md rounded-xl border border-zinc-200 bg-white p-6 shadow-xl dark:border-zinc-800 dark:bg-zinc-900">
        <div className="mb-4 flex items-center justify-between">
          <h2 id={titleId} className="text-lg font-semibold text-zinc-900 dark:text-zinc-50">Create Platform Admin</h2>
          <button
            type="button"
            onClick={handleClose}
